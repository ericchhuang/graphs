...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        for (int i = 0; i < segment.size() - 1; i++) {
            ...
            double distance = road.length();
            while (i + 2 < segment.size()
                    && _map.getLabel(segment.get(i + 1), segment.get(i + 2)).
                            toString().equals(road.toString())
                    && _map.getLabel(segment.get(i + 1), segment.get(i + 2)).
                            direction().equals(road.direction())) {
                distance = distance + _map.getLabel(segment.get(i + 1),
                        segment.get(i + 2)).length();
                i++;
            }
            String result = (seq + ". Take " + road + " "
                    + road.direction().fullName() + " for "
                    + (double) Math.round(distance * 10) / 10) + " miles.";
            if (i == segment.size() - 2) {
                result = result.substring(0, result.length() - 1);
                result += " to " + _map.getLabel(segment.get(i + 1)) + ".";
            }
