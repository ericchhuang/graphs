...
class Rule {
    ...
    void addDependency(Rule dependent) {
        int id = 0;
        for (int element : _depends.vertices()) {
            if (_depends.getLabel(element).equals(dependent)) {
                id = element;
            }
        }
        if (id == 0) {
            id = _depends.add(dependent);
        }
        _depends.add(_vertex, id);
    }
    ...
    private void checkFinishedDependencies() {
        for (Integer r : _depends.successors(_vertex)) {
            if (_depends.getLabel(r).isUnfinished()) {
                error("error: " + _depends.getLabel(r).getTarget()
                        + " is unfinished.");
            }
        }
    }
    ...
    private boolean outOfDate() {
        ...
        for (Integer r : _depends.successors(_vertex)) {
            if (_depends.getLabel(r).getTime() > _time) {
                return true;
            }
        }
        return false;
